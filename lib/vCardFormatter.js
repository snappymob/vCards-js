...
(function vCardFormatter() {
	...
	function getFormattedAddress(encodingPrefix, address) {
		...
		if (address.details.label ||
			...
			if (majorVersion >= 4) {
				formattedAddress = 'ADR' + encodingPrefix + ';TYPE=' + e(address.type) +
				...
				if (address.details.label) {
					formattedAddress = 'LABEL' + encodingPrefix + ';TYPE=' + e(address.type) + ':' + e(address.details.label) + nl();
				}
				formattedAddress += 'ADR' + encodingPrefix + ';TYPE=' + e(address.type) + ':;;' +
					e(address.details.street) + ';' +
					e(address.details.city) + ';' +
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
			...
			if (vCard.customPhone) {
				for (let key in vCard.customPhone) {
					if (vCard.customPhone.hasOwnProperty(key) && vCard.customPhone[key]) {
						if(!Array.isArray(vCard.customPhone[key])){
							vCard.customPhone[key] = [vCard.customPhone[key]];
						}
						vCard.customPhone[key].forEach((number, index) => {
							let label = 'T';
							if (vCard.customPhone[key].length > 1) {
								label = label + (index+1);
							}

							if (majorVersion >= 4) {
								formattedVCardString += 'TEL;VALUE=uri;TYPE="voice,' + label + ' - ' + e(key) + '":tel:' + e(number) + nl();
							} else {
								formattedVCardString += 'TEL;TYPE=' + label + ' - ' + e(key) + ',VOICE:' + e(number) + nl();
							}
						});
			...
			if (vCard.customFax) {
				for (let key in vCard.customFax) {
					if (vCard.customFax.hasOwnProperty(key) && vCard.customFax[key]) {
						if(!Array.isArray(vCard.customFax[key])){
							vCard.customFax[key] = [vCard.customFax[key]];
						}
						vCard.customFax[key].forEach((number, index) => {
							let label = 'F';
							if (vCard.customFax[key].length > 1) {
								label = label + (index+1);
							}

							if (majorVersion >= 4) {
								formattedVCardString += 'TEL;VALUE=uri;TYPE="fax,' + label + ' - ' + e(key) + '":tel:' + e(number) + nl();
							} else {
								formattedVCardString += 'TEL;TYPE=' + label + ' - ' + e(key) + ',FAX:' + e(number) + nl();
							}
						});
					}
				}
			...
			if (vCard.customEmail) {
				for (let key in vCard.customEmail) {
					if (vCard.customEmail.hasOwnProperty(key) && vCard.customEmail[key]) {
						if(!Array.isArray(vCard.customEmail[key])){
							vCard.customEmail[key] = [vCard.customEmail[key]];
						}
						vCard.customEmail[key].forEach((email, index) => {
							let label = 'E';
							if (vCard.customEmail[key].length > 1) {
								label = label + (index+1);
							}

							if (majorVersion >= 4) {
								formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + label + ' - ' + e(key) + ':' + e(email) + nl();
							} else if (majorVersion >= 3 && majorVersion < 4) {
								formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + label + ' - ' + e(key) + ',INTERNET:' + e(email) + nl();
							} else {
								formattedVCardString += 'EMAIL' + encodingPrefix + ';' + label + ' - ' + e(key) + ';INTERNET:' + e(email) + nl();
							}
						});
					}
				}
			...
			if (vCard.customUrl) {
				for (let key in vCard.customUrl) {
					if (vCard.customUrl.hasOwnProperty(key) && vCard.customUrl[key]) {
						if(!Array.isArray(vCard.customUrl[key])){
							vCard.customUrl[key] = [vCard.customUrl[key]];
						}
						vCard.customUrl[key].forEach((website, index) => {
							let label = 'W';
							if (vCard.customUrl[key].length > 1) {
								label = label + (index+1);
							}

							formattedVCardString += 'URL;TYPE=' + label + ' - ' + e(key) + encodingPrefix + ':' + e(website) + nl();
						});
					}
				}
			}

			if (vCard.customAddress) {
				for (let key in vCard.customAddress) {
					if (vCard.customAddress.hasOwnProperty(key) && vCard.customAddress[key]) {
						// console.log({details: vCard.customAddress[key], type:'WORK'});
						formattedVCardString += getFormattedAddress(encodingPrefix, {details: vCard.customAddress[key], type:key});
					}
				}
			}
			...
			if (vCard.url) {
				formattedVCardString += 'URL;TYPE=Website' + encodingPrefix + ':' + e(vCard.url) + nl();
			}
			...
			if (vCard.socialUrls) {
				for (var key in vCard.socialUrls) {
					if (vCard.socialUrls.hasOwnProperty(key) &&
						...
						formattedVCardString += 'URL;TYPE=' + key + ':' + e(vCard.socialUrls[key]) + nl();
					}
				}
			}
